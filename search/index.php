                    ...
                    <div class="space-y-2">
                        ...
                        <div id="result" class="[ mt-6 grid gap-x-2 gap-y-10   xl:gap-x-8 ] [ sm:grid-cols-2 ] [ lg:grid-cols-3 ] [ xl:grid-cols-4 ] [ 2xl:grid-cols-4 ]" style="font-family: Raleway;">
                            ...
                            if (isset($_GET['query'])) {
                                ...
                                $sql_count = "SELECT COUNT(*) as count FROM productos_agrupados WHERE categoria = $categoria"; //consutla para obtener la cantida de paginas que tiene la consulta
                                $count = mysqli_fetch_assoc(mysqli_query($conexion, $sql_count))['count'];
                                $data['num_pages'] = ceil($count / $results_per_page); //agregarmos la cantidad de paginas que tiene al array principal 
                                ...
                                if (count($data['result']) != 0) {
                                    ...
                                    for ($i = 0; $i < count($data['result']); $i++) {
                                        ...
                                        $precio = number_format($data['result'][$i]['precio'], 2);
                                        $simbolo = $data['result'][$i]['simbolo'];
                                        $stock = $data['result'][$i]['stock'];
                ...
                if (validar) {
                    ...
                    filteredArray.map((item) => {
                        ...
                        <tr class="border-b dark:border-neutral-500 bg-[#FBAA35]">
                          ...
                          <button data-te-ripple-init="" data-te-ripple-color="light" type="submit" class="opa hover:bg-warning-700 focus:bg-warning-700 active:bg-warning-800 relative rounded-r bg-black px-6 py-2.5 text-xs font-medium uppercase leading-tight text-white shadow-md transition duration-150 ease-in-out hover:shadow-lg focus:shadow-lg focus:outline-none focus:ring-0 active:shadow-lg" id="button-addon2">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-4 w-4">
                              <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 3h1.386c.51 0 .955.343 1.087.835l.383 1.437M7.5 14.25a3 3 0 00-3 3h15.75m-12.75-3h11.218c1.121-2.3 2.1-4.684 2.924-7.138a60.114 60.114 0 00-16.536-1.84M7.5 14.25L5.106 5.272M6 20.25a.75.75 0 11-1.5 0 .75.75 0 011.5 0zm12.75 0a.75.75 0 11-1.5 0 .75.75 0 011.5 0z"></path>
                            </svg>
                          </button>
