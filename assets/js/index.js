...
const idrandom = (length) => { //creador de id random
  let result = ''
...
debugger
    ...
    Carrito.list.map((item) => {
      ...
      $(`#${Carrito.id}`).append(
        ` 
      <div id="${item.ticketid}" class="[ relative  flex gap-1 items-center border-b p-1 border-gray-300   ]">
          <div class="absolute top-0 cursor-pointer " onclick='Carrito.delete("${item.ticketid}")'>
          ...
          <div class="[ w-full flex justify-between items-center ]">
              <div class="[ ml-5 flex gap-1 items-center   ]">
                  <div class="[ w-16 ]">
                      <img loading="lazy"  data-te-ripple-init onerror="this.onerror=null;this.src='./${item.imagen}';" src="../${item.imagen}" title="${item.nombre}" class="h-12  w-12   object-cover">
                  </div>
                  <div class="[ w-full text-slate-700 ]">
                      ${item.nombre.length >= 10 ? item.nombre.substring(0, 25) + "..." : item.nombre}
                  </div>

              </div>
              <div>
                  <p class="text-lg font-bold flex gap-1 items-center ">${item.precio}${item.simbolo}<label class='text-xs text-gray-500'> (${item.cantidad}x)</label></p>
              </div>
          </div>
      </div>
      `
  ...
  delete: (ticketid) => {
    Carrito.list.map((Item, index) => {
      ...
      if (Item.ticketid == ticketid) {
        ...
        $(`#${ticketid}`).hide("puff").queue(function () {
          $(`#${ticketid}`).remove();
        });
  ...
  makeRandomId: (length) => { //creador de id random
    let result = ''
    const characters = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789'
    for (let i = 0; i < length; i++) {
      result += characters.charAt(Math.floor(Math.random() * characters.length));
